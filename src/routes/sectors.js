import { db } from '../db';
import { players, economicEvents } from '../../drizzle/schema';
import { eq, and, gte, desc } from 'drizzle-orm';
export async function sectorsRoutes(fastify) {
    // GET /api/sectors
    fastify.get('/api/sectors', async (request, reply) => {
        ...
        for (const sector of allSectors) {
            ...
            const playerCount = await db
                ...
                .from(players)
                .where(and(eq(players.sector, sector), eq(players.isActive, true)));
            ...
            const sectorEvents = recentEvents.filter((e) => e.sectors.includes(sector));
